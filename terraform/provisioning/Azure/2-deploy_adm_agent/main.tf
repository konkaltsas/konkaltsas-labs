...
resource "azurerm_network_security_group" "terraform_security_group" {
  name                = "kk-terraform_security_group"
...
resource "azurerm_network_interface" "terraform_agent_interface" {
  name                = "kk-terraform_agent_interface"
  location            = var.location
  resource_group_name = data.azurerm_resource_group.resource_group.name
...
resource "azurerm_virtual_machine" "terraform-adm-agent" {
  ...
  os_profile {
    computer_name  = "kk-Citrix-ADM-Agent"
  ...
  storage_image_reference {
    ...
    sku       = "netscaler-ma-service-agent-141"
    version   = "latest"
  }

  plan {
    name      = "netscaler-ma-service-agent-141"
    publisher = "citrix"
